    ...
    intercept(request: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
        ...
        return of(null)
            ...
            function handleRoute() {
                switch (true) {
                    case url.endsWith('http://localhost:3000/users') && method === 'POST':
                        return authenticate();
                    case url.endsWith('http://localhost:3000/users/register') && method === 'POST':
                        return register();
                    case url.endsWith('http://localhost:3000/users') && method === 'GET':
                        return getUsers();
...
export const backendServiceProvider = {
    ...
    useClass: BackendServiceInterceptor,
    multi: true
};  